...
- **Причина:** на странице продукции не отображались изображения; кнопка «Показать ещё» в детальном режиме была неочевидна; в админке поля парсера показывались на английском; снятые галочки «Какие поля тянуть с Омолоко» не убирали старые значения у существующих товаров
- **Сделано:** парсер дополнен обогащением карточек из детальных страниц omoloko (image_url, описание, блок «О товаре/состав», штук в коробке); для существующих товаров при `update_existing=true` и снятых галочках невыбранные поля очищаются (кроме обязательных `article`, `name`, `sort_order`); в админке добавлен перевод названий полей; на фронтенде продукции добавлены блоки «Краткое описание» и «О товаре» в списке и деталке, скрыт пагинатор в деталке, добавлен `catch` для загрузки
- **Проверка:** `docker compose build backend frontend`; `docker compose up -d backend frontend nginx`; `python -m compileall app/services/parser.py`
- **Блокеры:** для проверки новых данных в БД нужен ручной запуск парсера из админки (автозапуск через скрипт заблокирован политикой безопасности среды)
- **Как проверить:** админ → Продукция → выбрать нужные поля + «Обновлять существующие товары» → «Запустить парсер»; пользователь → `/pages/products.html` (изображения, полный текст, корректная работа списка)

### Запись: 2026-06-23 (bugfix — парсер omoloko.ru, ошибка 500)
- **Чат:** bugfix — `ParserService`, `/api/parser/run`
- **Причина:** в логах backend — `StringDataRightTruncationError`: в `image_url` попадали длинные `data:image/svg+xml...` (плейсхолдеры), артикулы брались из slug URL вместо «Артикул: 10120430»
- **Сделано:** переписан парсер под реальную вёрстку omoloko.ru — поиск карточек по ссылкам `/catalog/icecream/...`, извлечение числового артикула, названия, группы, веса; игнорирование `data:`-картинок; обрезка полей под лимиты БД; автообновление `selectors_config` для omoloko
- **Проверка:** `docker compose build backend`; тестовый запуск парсера — `created_count: 100`, в БД 100 товаров; `/api/health` — 200
- **Блокеры:** нет
- **Как проверить:** админ → Продукция → «Запустить парсер»; пользователь → «Продукция ЧИСТАЯ ЛИНИЯ» — карточки товаров

### Запись: 2026-06-23 (Э11 — деплой Timeweb, подготовка прода)
- **Чат:** Э11 — деплой (VPS, DNS, HTTPS, deploy.sh)
- **Сделано:** заменена заглушка `scripts/deploy.sh` на рабочий скрипт деплоя (backup -> pull -> build -> migrate -> up -> health-check); добавлен `docker-compose.prod.yml` (80/443 + монтирование `/etc/letsencrypt` и `/.well-known`); добавлен `docker/nginx/nginx.prod.conf` (HTTP->HTTPS, SSL, security headers, reverse proxy); добавлен `scripts/setup-https.sh` для выпуска сертификата Let's Encrypt; добавлена пошаговая инструкция `DEPLOY-TIMEWEB.md`
- **Проверка:** проверка структуры изменений в git; запуск на боевом VPS и выпуск сертификата требует действий владельца (DNS и SSH-доступ)
- **Блокеры:** без VPS и DNS нельзя завершить финальную проверку HTTPS в этом чате
- **Следующий шаг:** на сервере выполнить `./scripts/setup-https.sh`, затем `HEALTHCHECK_URL=https://plombirclub.ru/api/health ./scripts/deploy.sh`
